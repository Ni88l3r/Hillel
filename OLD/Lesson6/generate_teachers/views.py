...
def create_teacher(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('generate_teachers.teachers'))
    else:
...
def create_student(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('generate_teachers.students'))
    else:
...
def create_group(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse('generate_teachers.groups'))
    else:
